...
void initWindow(int argc, char** argv) {
	...
	glutSetWindow(glutCreateWindow("Binary tree"));
...
void display(void) {
	...
	node.str = curLevel;
	Vi.push_back(node);
	glClearColor(0.6, 0.647, 0.658, 1.0);//Цвет фона
...
void FindMaxWay()
{
	cout << "Number of branches: " << curLevel - 1 << endl;
}
...
void mouseClick(int btn, int stat, int x, int y) {
	...
	if (stat == GLUT_DOWN) {
		...
		if (x > SHIFT && x < 200 && y > SHIFT + 80 && y < SHIFT + 110)
		{
			cout << "Vershina?\n";
			cin >> k;
			temp->RootColPath(k);
		}
		if (x > SHIFT && x < 200 && y > SHIFT + 120 && y < SHIFT + 150)
			FindMaxWay();
	}
	glutPostRedisplay();
}
