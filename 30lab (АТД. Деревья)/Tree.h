...
void Tree<T>::RootColPath(T k)
{
	...
	if (e)
	{
		...
		cout << "Kolichestvo reber ot kornya do zadannogo grafa: " << col << endl;
	}
	else
	{
...
void drawButtons()
{
	GLfloat TextButtons[3] = { 1,1,1 };
	GLfloat ButtonsColor[3] = { 0.293, 0.345, 0.36 };
	int shift = glutContext.shift;
	int height = glutContext.window_height;
	glColor3fv(ButtonsColor);
	glBegin(GL_QUADS);
	glVertex2i(shift, height - shift - 30);
	glVertex2i(shift + 200, height - shift - 30);
	glVertex2i(shift + 200, height - shift);
	glVertex2i(shift, height - shift);
	glEnd();
	glColor3fv(TextButtons);
	drawText1("Add element", shift, height - shift);
	...
	glVertex2i(shift + 200, height - shift - 110);
	glVertex2i(shift + 200, height - shift - 80);
	glVertex2i(shift, height - shift - 80);
	glEnd();
	glColor3fv(TextButtons);
	drawText1("Find the way to the root", shift, height - shift - 80);

	glColor3fv(ButtonsColor);
	glBegin(GL_QUADS);
	glVertex2i(shift, height - shift - 150);
	glVertex2i(shift + 200, height - shift - 150);
	glVertex2i(shift + 200, height - shift - 120);
	glVertex2i(shift, height - shift - 120);
	glEnd();
	glColor3fv(TextButtons);
	drawText1("Find max height", shift, height - shift - 120);
}

void drawFillCircle(int x, int y, int R)
{
	glColor3f(0.293, 0.345, 0.36);
	float x1, y1;
	glBegin(GL_POINTS);
	for (int i = 0; i <= R; i++)
	{
...
void drawText(T text, void* font, int text_x, int text_y, int R, int k)
{
	glColor3f(1.0, 1.0, 1.0);
	glPushMatrix();
	glTranslatef(text_x + 3, text_y + 3, 0.0);
