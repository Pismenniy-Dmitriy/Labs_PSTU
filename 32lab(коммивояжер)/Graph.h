...
void install(int***& mat, int& n, int**& help, int*& result)
{
	n = amountVerts;
...
void show(int*** mat, int n, int** help, int* result)
{
	install(mat, n, help, result);
	int s = 0;
...
void Graph<T>::InsertVertex(const T& vertex) {
	if (!this->IsFull()) {
		...
		cout << "The graph is already filled.Unable to add new vertex " << endl;
		return;
	}
}
...
Graph<int> makeGraph()
{
	...
	cout << "Enter the number of vertices of the graph: "; cin >> amountVerts; cout << endl;
	cout << "Enter the number of graph edges : "; cin >> amountEdges; cout << endl;
	...
	for (int i = 0; i < amountEdges; ++i) {
		cout << "Enter the start vertex: "; cin >> sourceVertex; cout << endl;
		int* sourceVertPtr = &sourceVertex;
		cout << "Enter the end vertex: "; cin >> targetVertex; cout << endl;
		int* targetVertPtr = &targetVertex;

		cout << "Enter edge weight : "; cin >> edgeWeight; cout << endl;
...
void Graph<T>::InsertEdge(const T& vertex1, const T& vertex2, int weight) {
	if (this->GetVertPos(vertex1) != (-1) && this->GetVertPos(vertex2) != (-1)) {
		...
		if (this->adjMatrix[vertPos1][vertPos2] != 0
			...
			cout << "An edge between the vertices is already exist" << endl;
			return;
		}
		else {
		...
		cout << "Both vertices (or one of them) are not in the graph " << endl;
		return;
	}
}
...
void mouseClick(int btn, int stat, int x, int y) {
	...
	if (stat == GLUT_DOWN) {
		...
		if (x > shift&& x < shift + 135 && y >  shift  && y < shift + 30)
		{
			...
			cout << "Enter vertex number : "; cin >> sourceVertex; cout << endl;
			int* sourceVertPtr = &sourceVertex;
			cout << "Enter the number of the vertex which the new vertex will be associated : "; cin >> targetVertex; cout << endl;
			int* targetVertPtr = &targetVertex;
		...
		if (x > shift&& x < shift + 135 && y > shift + 40 && y < shift + 70)
		{
			...
			cout << "Enter the number of deleted vertex : "; cin >> sourceVertex; cout << endl;
			int* sourceVertPtr = &sourceVertex;
			...
			else cout << "There is no such vertex in the graph.\n";
		}
